...
\subsection{LFO Units}

To provide an intuitive interface for the users of the synthesizer created for the purpose of this thesis, LFOs and LFO Sequences are packed into an "LFO Unit". Such an LFO Unit comprises two LFOs and two LFO Sequences, as well as a cross-fader. There is an LFO mode and an LFO Sequence mode between which the user can switch to display either the two LFOs or the two LFO Sequences. The cross-fader is used to mix between the signals of the two objects currently in display. For example, the LFO Unit can be configured to produce samples that are from an LFO with a sine wave and from another LFO with a sawtooth wave, each making up 50\% of the final signal.
...
\subsection{Crossfading with the $\sin$ function}
...
\subsection{Radical Crossfading}

\subsection{Scaling Crossfade values}

\subsection{Implementation}

\subsubsection{Pantables}

\subsubsection{The \texttt{CrossfadeUnit} class}

\subsubsection{The \texttt{Crossfader} class}
