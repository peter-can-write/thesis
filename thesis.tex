...
\begin{equation}
  z_{n} = (x_{n} \times (1 - dw)) + (y_{n} \times dw)
  \label{eq:drywet}
\end{equation}
...
The first effect that can be implemented very easily using just delay lines is the Echo effect. An echo is the result of summing an input sample with the output sample of a delay line. An implementation from the \texttt{Echo} class, derived from the \texttt{Delay} class, is shown in Table \ref{code:echo}.
